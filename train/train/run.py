...
# Value given by a fellow comrade.
Input_Shape = 768
...
def train(dataset_path, train_conf, model_path, add_timestamp):
    ...
    # TODO: CODE HERE
    # save labels index in artefacts_path/labels_index.jsonopen(f'train/data/artefacts/{artefacts_path}/params.json',
    # "train_output.json"), "w")
...
if __name__ == "__main__":
    ...
    parser.add_argument("train/data/training-data/stackoverflow_posts.csv", help="Path to training dataset")
    parser.add_argument("train/data/training-data/stackoverflow_posts.csv",
                        help="Path to Yaml file specifying training parameters")
    parser.add_argument("train/data/artefacts", help="Folder where training artefacts will be persisted")
    parser.add_argument("add_timestamp", action='store_true',
                        help="Create artefacts in a sub folder with name equal to execution timestamp")
